...
class ReceiptService {
              ...
              if (amountPaid != null && amountPaid > 0) ...[
                ...
                pw.Row(
                  ...
                  children: [
                    ...
                    pw.Text("N${amountPaid.toStringAsFixed(0)}", style: const pw.TextStyle(fontSize: 10)),
                  ],
                ),
                pw.Row(
                  mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                  children: [
                    pw.Text("BALANCE DUE:", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10, color: PdfColors.orange)),
                    pw.Text("N${(balance ?? 0).toStringAsFixed(0)}", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10, color: PdfColors.orange)),
                  ],
                ),
              ],
              pw.Divider(thickness: 0.5),
              pw.Text("Payment: ${paymentMethod.toUpperCase()}${balance != null && balance > 0 ? ' (PARTIAL)' : ''}", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9)),
  ...
  static Future<void> downloadReceiptImage({
    ...
    final images = Printing.raster(bytes, pages: [0], dpi: 300);

    await for (var image in images) {
      final pngBytes = await image.toPng();
      await Printing.sharePdf(
        bytes: pngBytes, 
        filename: 'receipt_${orderNumber.substring(orderNumber.length - 6)}.png'
      );
      break;
    }
  }

  static Future<void> downloadReceiptImageFromOrder(dynamic order, String resolvedBranchName) async {
    // resolve data
    ...
    await downloadReceiptImage(
      ...
      amountPaid: order is Map ? (order['amountPaid'] as num?)?.toDouble() : (order is OrderModel ? (order as OrderModel).amountPaid : 0.0),
      balance: order is Map ? (order['balance'] as num?)?.toDouble() : (order is OrderModel ? (order as OrderModel).balance : 0.0),
